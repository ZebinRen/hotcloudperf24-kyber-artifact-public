...
read_target_latencies_plot_ticks = ['50', '100×10$^{3}$']
...
read_target_latencies_ticks = ['0']
read_target_latencies_values = [0]
write_target_latencies_ticks = ['0']
write_target_latencies_values = [0]
...
workload_fio_options = [
    '--name=1 --rw=randread --cpus_allowed=1 --iodepth=1 --name=2 --rw=randwrite --cpus_allowed=2 --iodepth=256'
]
...
def exec_cmd(cmd):
    ...
    try:
        ...
        print(err.output)
        print(
            f"Error in start new subprocess {getframeinfo(currentframe()).filename}:{getframeinfo(currentframe()).lineno}"
        )
        exit(1)
    ...
    for cur_rep in range(rep_start, rep_end + 1):
        ...
        for cur_workload_name, cur_workload_option in zip(
            ...
            while len(cur_para_value_all) > 0:
                ...
                if debug:
                    exec_cmd('sleep 2s')  # wait for bpf to start
                else:
...
# read IOPS
if True:
    fig_save_path = 'r1-w256-read-iops.pdf'
...
# write IOPS
if True:
    fig_save_path = 'f2fs-r1-w256-write-iops.pdf'
...
# read latency
if True:
    fig_save_path = 'f2fs-r1-w256-read-latency-us.pdf'
