...
# write IOPS
if True:
    fig_save_path = 'r1-w256-write-iops.pdf'
    ...
    for i in range(len(read_target_latencies_ticks)):
        for j in range(len(write_target_latencies_ticks)):
            if (i, j) in [(0, 0), (8, 0), (8, 9), (0, 9), (8, 7), (4, 7)]:
                if heatmap[i][j] < color_threshold:
                    color = 'w'
                else:
                    color = 'black'
...
if True:
    ...
    # reset_color()
    fig, ax = plt.subplots(figsize=(12, 8))
    ax.set_xticks(np.arange(len(write_target_latencies_plot_ticks)),
                  labels=write_target_latencies_plot_ticks,
                  fontsize=heatmap_axis_tick_font_size)
    read_label_reverse = copy.deepcopy(read_target_latencies_plot_ticks)
    read_label_reverse.reverse()
