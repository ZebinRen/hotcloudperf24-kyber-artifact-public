...
read_target_latencies_plot_ticks = [
    '50', '100', '250', '500', '1×10$^{3}$', '2×10$^{3}$', '5×10$^{3}$',
    '10×10$^{3}$', '100×10$^{3}$'
]
write_target_latencies_plot_ticks = [
    '20', '50', '100', '250', '500', '1×10$^{3}$', '2×10$^{3}$', '5×10$^{3}$',
    '10×10$^{3}$', '100×10$^{3}$'
]
...
FIO_OPTIONS = '--size=100% --norandommap=1 --group_reporting=1 --direct=1 --allow_file_create=0 --output-format=json --bs=4k'.format(
...
# Print without standard deviation
# There should be a table:
# Row: R target latency. Column: W target latency

if True:
    # Print without standard deviation
    # There should be a table:
    # Row: R target latency. Column: W target latency
    print('####### Results without standard deviation:')
    print('READ IOPS')
                ...
                print(str(
                    round(
                      ...
                      ' (' + str(
                          round(
                              np.std(
                                  iops_read_results[cur_workload_name][index]),
                              1)) + ')' + ' & ',
                      end='')
                index += 1
            print('\\\\')
            ...
            for cur_write_tick in write_target_latencies_ticks:
                print(str(
                    round(
                        ...
                        1000, 1)) + ' (' + str(
                            round(
                                np.std(read_p99_lat_results[cur_workload_name]
                                       [index]) / 1000, 1)) + ')' + ' & ',
                      end='')
                index += 1
            print('\\\\')
            ...
            for cur_write_tick in write_target_latencies_ticks:
                print(str(
                    round(
                        ...
                        1000, 1)) + ' (' + str(
                            round(
                                np.std(write_p99_lat_results[cur_workload_name]
                                       [index]) / 1000, 1)) + ')' + ' & ',
                      end='')
                index += 1
            print('\\\\')
...
# read IOPS
if True:
    fig_save_path = 'r256-w256-read-iops.pdf'
    ...
    ax.set_xticks(np.arange(len(write_target_latencies_ticks)),
                  labels=write_target_latencies_plot_ticks,
    ...
    ax.set_yticks(np.arange(len(read_target_latencies_plot_ticks)),
                  labels=read_label_reverse,
                  fontsize=heatmap_axis_tick_font_size)
    plt.xlabel('Write target latency~($\mu$s)',
               fontsize=heatmap_axis_tick_font_size)
    plt.ylabel('Read target latency~($\mu$s)',
               fontsize=heatmap_axis_tick_font_size)
    ...
    for i in range(len(read_target_latencies_ticks)):
        for j in range(len(write_target_latencies_ticks)):
            if (i, j) in [(0, 0), (8, 0), (8, 9), (0, 9), (1, 0), (1, 5),
                          (1, 8), (8, 6), (2, 9)]:
...
# write IOPS
if True:
    fig_save_path = 'r256-w256-write-iops.pdf'
    ...
    ax.set_xticks(np.arange(len(write_target_latencies_ticks)),
                  labels=write_target_latencies_plot_ticks,
                  fontsize=heatmap_axis_tick_font_size)
    read_label_reverse = copy.deepcopy(read_target_latencies_plot_ticks)
    read_label_reverse.reverse()
    ax.set_yticks(np.arange(len(read_target_latencies_plot_ticks)),
                  labels=read_label_reverse,
                  fontsize=heatmap_axis_tick_font_size)
...
# read latency
if True:
    fig_save_path = 'r256-w256-read-latency-ms.pdf'
...
# write latency
if True:
    fig_save_path = 'r256-w256-write-latency-ms.pdf'
